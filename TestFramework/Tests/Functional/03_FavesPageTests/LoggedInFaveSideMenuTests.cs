...
namespace TestFramework.Tests.Functional.FavesPage
{
    ...
    public class LoggedInFaveSideMenuTests : BaseTest
    {
        ...
        public void MeniuLateral_Navigare_ArTrebuisSaRedirectionezeLaRutaCorecta(string menuAction, string path)
        {
            ExecutaClickMeniu(menuAction);
            DriverMgr.Wait(1);

            string expectedPath = AppRoutes.LocalPath + path;
            Assert.That(DriverMgr.GetUrl(), Is.EqualTo(expectedPath), $"Navigarea a esuat pentru: {menuAction}");
        }
        #endregion
    }
}
